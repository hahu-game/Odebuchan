...
1. TurnProcessorにべんきょうの処理を追加:
   - 元気-25
   - たべる時の重さバフ +10
   - StudyCombo +1、次回べんきょう時にたべる時の重さバフ += (StudyCombo × 10)
   - べんきょう以外の行動でStudyCombo = 0
2. ログに連続ボーナスを表示

**確認項目**:
- [ ] べんきょうを実行すると、元気が25減少、たべる時の重さバフが10増加すること
- [ ] 2回連続でべんきょうを実行すると、2回目はたべる時の重さバフが20増加すること
...
### 7.8 UIController: 特殊能力ボタンと進化前表示の実装　※※※実装は保留中。※※※
...
### 9.1 元気不足時に行動が選択できない処理

**目的**: 選択フェーズにおいて、元気が足りない（実行すると元気がマイナスになってしまう）場合、その選択肢を選択できない状態にする。
         　※ジャンケンにより実行フェーズ中に元気が減った場合、選択フェーズ時点では元気が足りていたが、
         　　実行フェーズ中に元気が足りなくなる場合があるが、その場合は、元気の値をマイナスにして元々予定していた行動を実行する。

**作業内容**:
1. UIControllerで選択フェーズ開始時に各行動ボタンの元気チェックを実装:
   - 各行動に必要な元気があるか確認
   - 元気が足りない場合は、そのボタンを無効化（選択できない状態にする）
   - 視覚的に元気不足を表示（ボタンをグレーアウトするなど）
2. 実行フェーズ中に元気が足りなくなる場合の処理:
   - 選択フェーズ時点では元気が足りていたが、実行フェーズ中に元気が足りなくなる場合は、元気の値をマイナスにして元々予定していた行動を実行する（既存の実装で対応）
...
3. 投了ボタンで自分を敗北扱いにする処理を実装。投了ボタンが押された場合、即座にTurnProcessor.CheckVictory()を呼ぶ。
　　TurnProcessor.CheckVictory()に投了ボタンが押された場合は、終了条件の体重を満たしていなくても、相手の勝利となるロジックを追加する。

**確認項目**:
- [ ] 設定ボタン（SettingButton）押下時にSettingPanelが表示されること
- [ ] 音量スライダーでBGM・SEの音量が変更されること
- [ ] 投了ボタンで自分が敗北してゲームが終了すること

**参照スクリプト**: 「SettingController.cs」に上記処理を実装する。
...
### 9.4 相手のステータスパネルでの行動効果予測表示　※未完了　実装なしでもよくないか。。？
