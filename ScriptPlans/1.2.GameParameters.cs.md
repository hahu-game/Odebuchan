# 1.2 GameParameters.cs

## ファイルパス
`Assets/Scripts/Data/GameParameters.cs`

## 目的
ゲーム全体のパラメーターを一元管理するScriptableObjectを作成する
...
### アセットの作成手順
1. Unityエディタで、Project > Assets > Data フォルダを作成（なければ）
2. Assets > Data フォルダを右クリック
3. Create > Odebuchan > GameParameters を選択
4. 作成されたアセットの名前を "GameParameters" にする
5. Inspectorで各パラメーターを調整
...
public class GameManager : NetworkBehaviour
{
    ...
    {
        // パラメーターを使用
        int initialWeight = gameParams.InitialWeight;
        int eatEnergyChange = gameParams.EatEnergyChange;
    }
}
```

## 注意点
- このScriptableObjectは1つだけ作成し、GameManagerやTurnProcessorから参照する
- ゲーム実行中にパラメーターを変更してもすぐに反映される（デバッグに便利）
- OnValidate()で異常値をチェックしているため、Inspector上で適切な値を設定すること
