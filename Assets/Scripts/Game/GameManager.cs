...
/// 2体のUyopyonStateオブジェクトをPlayerRefで管理する。
/// </summary>
public class GameManager : NetworkBehaviour
{
    ...
    public NetworkObject uyopyonPrefab; // UnityエディタでUyopyonStateを持つプレハブをアサイン
    ...
    private void Awake()
    {
        ...
        {
            Instance = this;
            // シーンをまたぐ場合、DontDestroyOnLoad(gameObject); を追加
        }
    }

    /// <summary>
    /// NetworkPlayer.Spawned()から呼ばれる。ホスト側でのみ実行。
    /// 参加プレイヤーに対応するうーぴょんオブジェクトを生成する。
    /// </summary>
    //[Server] // ホストでのみ実行されることを保証
    public void SpawnUyopyon(PlayerRef player, string playerName)
    {
        ...
        NetworkObject newUyopyon = Runner.Spawn(
            ...
            inputAuthority: player // 入力権をこのプレイヤーに渡す
        );
